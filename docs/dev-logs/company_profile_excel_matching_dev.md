...
- 完整数据库迁移脚本，当前仍依赖项目现有 `create_all` 初始化方式。
...
- 增加 Excel 导入前预览，先展示“将导入多少条、过期多少条、异常多少条”，确认后再写入。
...
- 在分析详情页展示“命中证据”和“缺失证据”，而不只在后端评分详情中保留。
